...
function Debouncer(spec) {
  ...
  if (typeof spec.value !== 'string' && !Array.isArray(spec.value)) {
    throw new TypeError('Expected a string or Array, got ' + typeof spec.value);
  }
...
Debouncer.prototype._requestTick = function () {
  if (Array.isArray(this.value)) {
    ...
    for (var i = 0, l = this.value.length; i < l; i++) {
      this._last[this.value[i]] = this.elementWithValue[this.value[i]];
    }
  } else {
 ...
 * @returns {Debouncer}
 */
Debouncer.prototype.listen = function (runStack) {
...
Debouncer.prototype.runStack = function (current) {
  var i = 0
    , l = this._calls.length;
  for (i; i < l; i++) {
    if (typeof this._calls[i] === 'function') {
